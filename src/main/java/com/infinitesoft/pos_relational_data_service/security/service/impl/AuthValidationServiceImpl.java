...
public class AuthValidationServiceImpl implements AuthValidationService {
    ...
    public AuthUserDto validateToken(String token) {
        ...
        try {
            ...
            Mono<Boolean> monoBool = webClient.post()
                    ...
                    .body(BodyInserters.fromValue(Collections.singletonMap("token", token)))
                    .retrieve()
    ...
    private AuthUserDto parseUserFromJwt(String token) {
        try {
            ...
            if (nombre instanceof String) user.setNombre((String) nombre);

            Object sub = payload.get("sub");
            if (sub instanceof String) user.setCorreoElectronico((String) sub);

            Object telefono = payload.get("telefono");
            if (telefono instanceof String) user.setTelefono((String) telefono);
            ...
            if (rolesObj instanceof List) {
                ...
                for (Object r : list) {
                    ...
                    if (role.getSigla() != null && !role.getSigla().trim().isEmpty()) {
                        rolesOut.add(role);
                    }
                }
            }
